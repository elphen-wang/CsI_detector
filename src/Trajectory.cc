...
void Trajectory::MergeTrajectory(G4VTrajectory* secondTrajectory) {
    ...
    // 将第二个轨迹的点合并到当前轨迹
    fPositionRecord.insert(fPositionRecord.end(), 
                          second->fPositionRecord.begin(), 
...
void Trajectory::ShowTrajectory(std::ostream& os) const {
    os << "Trajectory: TrackID=" << fTrackID 
       << ", ParentID=" << fParentID
       << ", Particle=" << fParticleName
       << ", PDGEncoding=" << fPDGEncoding
       << ", Charge=" << fCharge / CLHEP::eplus << " e"
       << ", Points=" << fPositionRecord.size()
       << G4endl;
}
